...
for (s in seasons) {
  ...
  TE_cumulative_career_stats <- player_data %>% 
    filter(position == 'TE') %>% 
    filter(player_id %in% next_season_ids) %>% 
  ...
  # Put it all together
  TE_output <- TE_cumulative_career_stats %>% 
    inner_join(., TE_season_stats, by=c('player_id', 'player_display_name')) %>% 
    inner_join(., TE_target_stats, by=c('player_id', 'player_display_name')) %>% 
    dplyr::select(player_id, player_display_name, target_season, everything())
  
  # add to list
  if (length(TE_stat_lst) == 0) {
    TE_stat_lst <- list(TE_output)
  } else {
    TE_stat_lst <- c(TE_stat_lst, list(TE_output))
  }
}
...
# Combine Results
################################################################################
season_training_data <- do.call(rbind, TE_stat_lst)
...
# Put it all together
TE_external_data <- TE_cumulative_career_stats %>% 
  inner_join(., TE_season_stats, by=c('player_id', 'player_display_name')) %>% 
...
# Save Results
################################################################################
data.table::fwrite(season_training_data, "data/processed/season/te_season_stat_modeling_data.csv")
data.table::fwrite(TE_external_data, "data/processed/season/te_season_external.csv")
