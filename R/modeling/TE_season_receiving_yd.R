...
modeling_data <- data.table::fread("data/processed/season/te_season_stat_modeling_data.csv")
external_data <- data.frame(data.table::fread("data/processed/season/te_season_external.csv"))
...
y_train <- modeling_data[train_indices, ] %>% 
  dplyr::select(matches("target_receiving_yd")) %>% 
  data.frame()
...
# Create formula
formula_str <- paste("target_receiving_yd ~", paste0(numeric_cols, collapse = " + "))
...
cv_preds %>% 
  ...
  labs(x = 'Observed Rushing yd',
       y = 'Predicted Rushing yd',
...
prediction_df <- cbind(test_data[c('player_id', 'player_display_name', 'target_receiving_yd')],
                       test_predictions) %>% 
  rename(obs_receiving_yd = target_receiving_yd,
         pred_receiving_yd = fit,
...
# Statistics
reg_r2 <- summary(lm("pred_receiving_yd ~ obs_receiving_yd", data = prediction_df))$r.squared
rmse <- caret::RMSE(prediction_df$pred_receiving_yd, prediction_df$obs_receiving_yd)

print(paste0("Regression R-Squared: ", round(reg_r2, 2)))
print(paste0("RMSE: ", round(rmse, 2)))

# Plotting
pred_range <- range(prediction_df$pred_receiving_yd, prediction_df$obs_receiving_yd)

prediction_df %>% 
  ggplot() +
  geom_point(aes(x = round(obs_receiving_yd), 
                 y = round(pred_receiving_yd))) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  xlim(0, pred_range[2]) +
  ylim(0, pred_range[2]) +
  labs(x = 'Observed Receiving yd',
       y = 'Predicted Receiving yd',
       title = 'Receiving yd Prediction Performance')
...
# Save
data.table::fwrite(player_df, "data/results/TE/exp_t_receiving_yd_2023.csv")
