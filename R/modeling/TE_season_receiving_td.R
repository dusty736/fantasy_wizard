...
y_train <- modeling_data[train_indices, ] %>% 
  dplyr::select(matches("target_receiving_td")) %>% 
  data.frame()
...
# Create formula
formula_str <- paste("target_receiving_td ~", paste0(numeric_cols, collapse = " + "))
...
prediction_df <- cbind(test_data[c('player_id', 'player_display_name', 'target_receiving_td')],
                       test_predictions) %>% 
  rename(obs_receiving_td = target_receiving_td,
         pred_receiving_td = fit,
...
# Statistics
reg_r2 <- summary(lm("pred_receiving_td ~ obs_receiving_td", data = prediction_df))$r.squared
rmse <- caret::RMSE(prediction_df$pred_receiving_td, prediction_df$obs_receiving_td)

print(paste0("Regression R-Squared: ", round(reg_r2, 2)))
print(paste0("RMSE: ", round(rmse, 2)))

# Plotting
pred_range <- range(prediction_df$pred_receiving_td, prediction_df$obs_receiving_td)

prediction_df %>% 
  ggplot() +
  geom_point(aes(x = round(obs_receiving_td), 
                 y = round(pred_receiving_td))) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  xlim(0, pred_range[2]) +
  ylim(0, pred_range[2]) +
  labs(x = 'Observed Receiving td',
       y = 'Predicted Receiving td',
       title = 'Receiving td Prediction Performance')
...
# Save
data.table::fwrite(player_df, "data/results/TE/exp_t_receiving_td_2023.csv")
