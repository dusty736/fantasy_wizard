...
test_predictions <- data.frame(pred_receiving_td = predict(final_lm_model, test_data, interval = 'prediction'))
...
prediction_df <- cbind(test_data[c('player_id', 'player_display_name', 'target_receiving_td')],
  ...
  rename(obs_receiving_td = target_receiving_td,
         pred_receiving_td = pred_receiving_td)
...
player_df <- model_train(n_boot=100, 
                         ...
                         target_col_regex="target_receiving_td")

# Save
data.table::fwrite(player_df, "data/results/RB/exp_r_receiving_td_2023.csv")
